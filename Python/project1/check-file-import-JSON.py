...
# now we won't put shebang bin bash, but run it in bash via python3 <filename>
# the reciepts are read from the new folder, in the ubuntu server

import glob # this lib searches for a path name we defined, and returns the files list
import os # to create the folder
import shutil # library to work on files- copy, move
import json
...
print(receipts) # list of recietps from the glob

subtotal = 0.0 # float as we know
for path in receipts:
    with open(path) as f: # it opens the file one by one according to reciepts list
        content = json.load(f) # json.load is special function that i upload the content of json into "content", f is what
        #print(content) # we iterate over the paths
        subtotal += float(content['receipt']['purchasedItems'][0]['price']) # [0] because the price is inside the block of purchased items
